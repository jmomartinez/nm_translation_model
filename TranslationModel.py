...
class TranslationModel:
    ...
    def train(self, model: Sequential) -> Any:
        """
        ...
        if self.args.validation_split:
            fit_model = model.fit(self.x_train, self.y_train, epochs=self.args.epochs, batch_size=self.args.batch_size,
                                  validation_split=self.args.validation_split, verbose=2, callbacks=[chk_point])
        else:
            fit_model = model.fit(self.x_train, self.y_train, epochs=self.args.epochs, batch_size=self.args.batch_size,
                                  verbose=2, callbacks=[chk_point])

        self._output_model(fit_model)
        return fit_model
    ...
    def _output_model(fit_model: Any) -> None:
        ...
        path = f'output_files/fit_nmt_model_{now}.pkl'
